...
namespace HabboIM.Core
{
    public sealed class Logging
    {
        ...
        internal static void LogException(string logText)
        {
            try
            {
                ...
                /*if (!logText.Contains("Unknown baseID"))
                {
                    try
                    {
                        if (int.Parse(HabboIM.GetConfig().data["automatic-error-report"]) == 1)
                        {
        ...
        {
            Logging.WriteLine("[EXCEPTION] " + logText, ConsoleColor.Blue);
        }
        ...
        internal static void LogSQLUpdaterError(string logText)
        {
            try
            {
                FileStream fileStream = new FileStream("sqlupdaterexceptions.err", FileMode.Append, FileAccess.Write);
                byte[] bytes = Encoding.ASCII.GetBytes(string.Concat(new object[]
                {
                    DateTime.Now,
                    ": ",
                ...
                fileStream.Close();
                Console.ForegroundColor = ConsoleColor.Red;
                Logging.WriteLine("SQL Updater error saved");
                Console.ForegroundColor = ConsoleColor.Gray;
            }
            catch (Exception)
            {
