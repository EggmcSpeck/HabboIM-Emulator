		...
		public void Handle(GameClient Session, ClientMessage Event)
		{
            ...
            if (@class != null && @class.CheckRights(Session, true))
			{
				...
				if (class2 != null)
				{
					...
					if (text2 != null)
					{
						if (!(text2 == "floor"))
						{
							...
							else
							{
                                ...
                                if (Session.GetHabbo().CurrentQuestId > 0 && HabboIM.GetGame().GetQuestManager().GetQuestAction(Session.GetHabbo().CurrentQuestId) == "PLACEWALLPAPER")
								{
						...
						else
						{
                            ...
                            if (Session.GetHabbo().CurrentQuestId > 0 && HabboIM.GetGame().GetQuestManager().GetQuestAction(Session.GetHabbo().CurrentQuestId) == "PLACEFLOOR")
							{
                                HabboIM.GetGame().GetQuestManager().ProgressUserQuest(Session.GetHabbo().CurrentQuestId, Session);
							}
						}
					...
					HabboIM.GetGame().GetRoomManager().method_18(@class.Id);
				}
			}
		}
	}
