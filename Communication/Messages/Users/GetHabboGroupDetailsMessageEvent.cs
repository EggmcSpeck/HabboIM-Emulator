...
namespace HabboIM.Communication.Messages.Users
{
	internal sealed class GetHabboGroupDetailsMessageEvent : Interface
	{
		public void Handle(GameClient Session, ClientMessage Event)
		{
            ...
            Session.SendNotification("GetHabboGroupDetailsMessageEvent");
			...
			if (num > 0 && (Session != null && Session.GetHabbo() != null))
			{
				...
				if (@class != null)
				{
					...
					if (@class.uint_0 > 0u)
					{
						...
						if (HabboIM.GetGame().GetRoomManager().GetRoom(@class.uint_0) != null)
						{
							Message.AppendStringWithBreak(HabboIM.GetGame().GetRoomManager().GetRoom(@class.uint_0).Name);
							goto IL_15A;
						}
						using (DatabaseClient class2 = HabboIM.GetDatabase().GetClient())
						{
							try
							{
								...
								string string_ = HabboIM.GetGame().GetRoomManager().method_17(@class.uint_0, dataRow_).Name;
