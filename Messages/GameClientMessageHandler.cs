...
namespace HabboIM.Messages
{
	internal sealed class GameClientMessageHandler
	{
		...
		public void method_4()
		{
            if (this != null)
            {
                RoomAdvertisement @class = HabboIM.GetGame().GetAdvertisementManager().method_1();
		...
		{
			this.method_7();
			if (HabboIM.GetGame().GetRoomManager().method_12(uint_0) != null)
			{
                if (this.Session != null && this.Session.GetHabbo() != null && this.Session.GetHabbo().InRoom)
				{
					Room @class = HabboIM.GetGame().GetRoomManager().GetRoom(this.Session.GetHabbo().CurrentRoomId);
					if (@class != null)
					{
						@class.method_47(this.Session, false, false);
					}
				}
				Room class2 = HabboIM.GetGame().GetRoomManager().method_15(uint_0);
				if (class2 != null && Session != null && Session.GetHabbo() != null)
				{
					...
					if (class2.UsersNow >= class2.UsersMax && !HabboIM.GetGame().GetRoleManager().method_1(this.Session.GetHabbo().Rank, "acc_enter_fullrooms") && !this.Session.GetHabbo().IsVIP)
					{
						ServerMessage Message = new ServerMessage(224u);
					...
					else
					{
						if (class2.Type == "public")
						{
							...
							{
								this.Session.SendNotification("This public room is accessible to HabboIM staff only.");
								ServerMessage Message2 = new ServerMessage(18u);
								this.Session.SendMessage(Message2);
								return;
							}
		...
		{
			Room @class = HabboIM.GetGame().GetRoomManager().GetRoom(this.Session.GetHabbo().uint_2);
