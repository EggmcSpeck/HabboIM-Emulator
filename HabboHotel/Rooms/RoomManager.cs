    ...
    internal sealed class RoomManager
    {
        ...
        internal int Int32_1
        {
            get
            {
                ...
                {
                    result = int.Parse(@class.ReadString("SELECT COUNT(*) FROM rooms", 30));
                }
                return result;
            }
        }
        ...
        internal void method_0()
        {
            ...
            using (DatabaseClient @class = HabboIM.GetDatabase().GetClient())
            {
                for (int i = 0; i < 12; i++)
                {
                    DataTable dataTable = @class.ReadDataTable("SELECT * FROM rooms WHERE category = '" + i + "' AND roomtype = 'private' ORDER BY users_now DESC LIMIT 40", 30);
        ...
        private RoomModel method_9(uint uint_0)
        {
            ...
            using (DatabaseClient @class = HabboIM.GetDatabase().GetClient())
            {
                dataRow = @class.ReadDataRow("SELECT doorx,doory,height,modeldata FROM room_models_customs WHERE roomid = '" + uint_0 + "'", 30);
            }
        ...
        public RoomModel GetModel(string Model, uint uint_0)
        {
            ...
            else if (this.Models.ContainsKey(Model))
            {
        ...
        public RoomData method_12(uint uint_0)
        {
            ...
            lock (this.hashtable_0)
            {
                ...
                {
                    dataRow = class2.ReadDataRow("SELECT * FROM rooms WHERE Id = '" + uint_0 + "' LIMIT 1", 30);
                }
                if (dataRow == null)
                {
                    result = null;
        ...
        internal void UnloadAllRooms()
        {
            using (Class26 class26_ = this.class25_0.Class26_0)
            {
                foreach (Room @class in class26_.Values)
                {
                    if (@class != null)
                    {
                        HabboIM.GetGame().GetRoomManager().method_16(@class);
                    }
                }
            }
        }
