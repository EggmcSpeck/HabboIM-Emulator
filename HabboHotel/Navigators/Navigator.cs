...
namespace HabboIM.HabboHotel.Navigators
{
	internal sealed class Navigator
	{
		...
		public ServerMessage method_6(GameClient Session)
		{
			...
			using (TimedLock.Lock(Session.GetHabbo().list_1))
			{
				...
				{
					HabboIM.GetGame().GetRoomManager().method_11(current).method_3(Message, false, false);
				}
			}
			return Message;
		}
		...
		public ServerMessage method_8(GameClient Session, int int_0)
		{
			...
			Message.AppendStringWithBreak("");
			List<Room> list = HabboIM.GetGame().GetRoomManager().method_6(int_0);
			Message.AppendInt32(list.Count);
		...
		internal byte[] SerializeNavigator(GameClient Session, int int_0)
		{
			...
			{
				byte[] array = HabboIM.GetGame().GetNavigatorCache().method_1(int_0);
				if (array != null)
				{
					result = array;
				}
		...
		public ServerMessage method_12(GameClient Session, int int_0)
		{
			...
			default:
			{
				Dictionary<Room, int> dictionary = HabboIM.GetGame().GetRoomManager().method_21();
				IEnumerable<RoomData> arg_11F_0 = HabboIM.GetGame().GetRoomManager().list_3;
				if (func == null)
				{
			...
			foreach (MessengerBuddy class2 in hashtable.Values)
			{
				...
				{
					GameClient class3 = HabboIM.GetGame().GetClientManager().method_2(class2.UInt32_0);
			...
			try
			{
				while (enumerator3.MoveNext())
				{
					...
					list.Add(HabboIM.GetGame().GetRoomManager().method_17((uint)dataRow["Id"], dataRow));
				}
				goto IL_508;
			}
			finally
			...
			IL_47E:
			Dictionary<Room, int> dictionary4 = HabboIM.GetGame().GetRoomManager().method_21();
