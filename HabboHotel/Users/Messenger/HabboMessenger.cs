...
namespace HabboIM.HabboHotel.Users.Messenger
{
    internal sealed class HabboMessenger
    {
        ...
        internal void method_10()
        {
            using (DatabaseClient @class = HabboIM.GetDatabase().GetClient())
            {
                @class.ExecuteQuery("DELETE FROM messenger_requests WHERE to_id = '" + this.uint_0 + "'", 30);
            }
            this.method_3();
        }
        ...
        internal void method_11(uint uint_1)
        {
            using (DatabaseClient @class = HabboIM.GetDatabase().GetClient())
            {
                ...
                @class.ExecuteQuery("DELETE FROM messenger_requests WHERE to_id = @userid AND from_id = @fromid LIMIT 1", 30);
            }
        ...
        internal void method_12(uint uint_1)
        {
            using (DatabaseClient @class = HabboIM.GetDatabase().GetClient())
            {
                ...
                @class.AddParamWithValue("userid", this.uint_0);
                @class.ExecuteQuery("INSERT INTO messenger_friendships (user_one_id,user_two_id) VALUES (@userid,@toid)", 30);
                @class.ExecuteQuery("INSERT INTO messenger_friendships (user_one_id,user_two_id) VALUES (@toid,@userid)", 30);
            }
            this.method_14(uint_1);
            GameClient class2 = HabboIM.GetGame().GetClientManager().method_2(uint_1);
        ...
        internal void method_13(uint uint_1)
        {
            using (DatabaseClient @class = HabboIM.GetDatabase().GetClient())
            {
                @class.AddParamWithValue("toid", uint_1);
                @class.AddParamWithValue("userid", this.uint_0);
                @class.ExecuteQuery("DELETE FROM messenger_friendships WHERE user_one_id = @toid AND user_two_id = @userid LIMIT 1", 30);
                @class.ExecuteQuery("DELETE FROM messenger_friendships WHERE user_one_id = @userid AND user_two_id = @toid LIMIT 1", 30);
            }
        ...
        internal void method_14(uint uint_1)
        {
            using (DatabaseClient @class = HabboIM.GetDatabase().GetClient())
            {
                DataRow dataRow = @class.ReadDataRow("SELECT username,motto,look,last_online FROM users WHERE Id = '" + uint_1 + "' LIMIT 1", 30);
        ...
        internal void method_18(uint uint_1, string string_0)
        {
            ...
            {
                GameClient @class = HabboIM.GetGame().GetClientManager().method_2(uint_1);
                if (@class == null || @class.GetHabbo().GetMessenger() == null)
                {
                    this.method_20(5, uint_1);
                }
                else if (this.method_25().GetHabbo().IsMuted)
                {
                ...
                else
                {
                    ...
                    string antiweberstring = string_0;
                    ...
                    {
                        string string_aw = ChatCommandHandler.smethod_4b(this.method_25(), antiweberstring, "Chat");
                    }
                    if (this.method_25().GetHabbo().jail == 1)
                    {
                        //string_0 = "Ich bin ein Inhaftierter User und kann deshalb keine Nachrichten versenden.";
                        this.method_25().SendNotification("Du bist gebannt und kannst keine Nachrichten versenden!");
                    }

                    if (this.method_25().GetHabbo().jail == 0)
                    {
        ...
        internal ServerMessage method_24(string string_0)
        {
            ...
            using (DatabaseClient @class = HabboIM.GetDatabase().GetClient())
            {
                ...
                dataTable = @class.ReadDataTable("SELECT Id FROM users WHERE username LIKE @query LIMIT 50", 30);
            }
            ...
            using (DatabaseClient @class = HabboIM.GetDatabase().GetClient())
            {
                ...
                {
                    uint num = (uint)dataRow["Id"];
                    DataRow dataRow2 = @class.ReadDataRow("SELECT username,motto,look,last_online FROM users WHERE Id = '" + num + "' LIMIT 1", 30);
        ...
        {
            return HabboIM.GetGame().GetClientManager().method_2(this.uint_0);
        }
