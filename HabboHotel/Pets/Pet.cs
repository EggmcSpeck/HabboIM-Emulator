...
namespace HabboIM.HabboHotel.Pets
{
	internal sealed class Pet
	{
		...
		public Room Room
		{
			get
			{
				if (!IsInRoom)
				{
					...
					return HabboIM.GetGame().GetRoomManager().GetRoom(RoomId);
			}
		}
		...
		public int Age
		{
			get
			{
				return (int)Math.Floor((HabboIM.GetUnixTimestamp() - this.CreationStamp) / 86400.0);
			}
		}
		public string Look
		{
		...
		public string OwnerName
		{
			get
			{
				return HabboIM.GetGame().GetClientManager().GetNameById(OwnerId);
			}
		}
		...
		public void AddExpirience(int Amount, int int_9)
		{
			...
			if (this.Expirience < 51900)
			{
                ...
                if (this.Room != null && LevelUp)
                {
                    ...
                    message.AppendStringWithBreak("*leveled up to level " + (this.Level) + "*");
